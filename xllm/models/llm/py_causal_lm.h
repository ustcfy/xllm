...
class __attribute__((visibility("hidden"))) PyCausalLM : public CausalVLM {
 public:
  ...
  // DSpark ConfidenceHead path (return_selected_hidden). The Python lm_head
  // exposes only logits, so out_hidden is left undefined and the worker
  // gathers selected hidden from hidden_states.
  torch::Tensor logits(const torch::Tensor& hidden_states,
