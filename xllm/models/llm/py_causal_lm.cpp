...
torch::Tensor PyCausalLM::logits(const torch::Tensor& hidden_states,
  ...
  // The Python lm_head returns only logits; out_hidden is intentionally left
  // undefined so the caller gathers selected hidden from hidden_states (see the
  // return_selected_hidden fallback in llm_worker_impl.cpp).
