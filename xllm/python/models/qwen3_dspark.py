...
Extends the DFlash backbone (see ``qwen3_dflash``) with Markov + optional
...
class DSparkForCausalLM(PyModelBase):
    ...
    def dspark_confidence_probs(
        ...
        markov_embedding = self.markov_head.embed(prev_matrix) if self.confidence_head.with_markov else None
    ...
    def load_weights(self, state_dicts: list, tp_rank: int, tp_size: int) -> None:
        ...
        loader = WeightLoader(self, state_dicts, tp_size, tp_rank, kv_world=kv_world, kv_rank=kv_rank)
        shard = loader.load_shard

        # embed_tokens / lm_head: the draft owns its own copies.
        loader.copy_in("model.embed_tokens.weight", shard("embed_tokens.weight", dim=1))
        loader.copy_in("lm_head.weight", shard("lm_head.weight", dim=0))
        ...
        loader.copy_in("model.fc.weight", fc_weight)
        loader.copy_in("model.hidden_norm.weight", loader.load_tensor("hidden_norm.weight"))
        ...
        if self.markov_head is not None:
            loader.copy_in("markov_head.markov_w1.weight", loader.load_tensor("markov_head.markov_w1.weight"))
            loader.copy_in("markov_head.markov_w2.weight", loader.load_tensor("markov_head.markov_w2.weight"))

        if self.confidence_head is not None:
            loader.copy_in("confidence_head.proj.weight", loader.load_tensor("confidence_head.proj.weight"))
            bias_sd = loader.find("confidence_head.proj.bias")
            if bias_sd is not None:
                loader.copy_in("confidence_head.proj.bias", bias_sd.get_tensor("confidence_head.proj.bias"))
            else:
