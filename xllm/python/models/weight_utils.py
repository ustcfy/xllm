...
def moe_shard(cfg) -> tuple[int, int]:
...
class WeightLoader:
    ...
    def __init__(
        ...
        self._tensors_by_name = dict(model.named_parameters())
    ...
    def _resolve(self, name: str) -> Optional[tuple[StateDict, str]]:
        """Return ``(state_dict, resolved_name)`` for the first ``src_prefix + alias`` present.

        For each alias, tries every ``src_prefix`` in prepend mode first, then
        strip mode (drop the prefix from ``alias`` if it starts with it) — this
        mirrors the historical two-directional MTP name aliasing.
        """
    ...
    def shard(
        ...
        return t.narrow(dim, rank * chunk_size, chunk_size).contiguous()
    ...
    def copy_shard(self, name: str, dim: int, world: Optional[int] = None, rank: Optional[int] = None) -> None:
        """Load ``name``, shard on ``dim``, and copy into the same-named param."""
        self.copy_in(name, self.load_shard(name, dim, world=world, rank=rank))
...
class W8A8WeightLoader(WeightLoader):
    ...
    def load_w8a8_a(self, prefix: str, proj: str, shard_dims: Optional[dict[str, int]] = None) -> None:
    ...
    def assert_moe_layer_symmetric_int8(self, layers: Sequence[nn.Module]) -> None:
        """Assert every local expert of every routed-MoE layer has zero W8A8 ``weight_offset``.

        Duck-typed on ``local_expert_start`` / ``local_expert_end`` so any MoE
        class works; no-op if none present. The routed-expert path drops
        offsets in :meth:`DeepseekV3MoE.load_experts`, so this check is the
        only gate protecting ``grouped_moe`` from a silently-asymmetric expert.
        """
        for i, layer in enumerate(layers):
            moe = getattr(layer, "mlp", None)
            if getattr(moe, "local_expert_start", None) is None:
                continue
            for expert_idx in range(moe.local_expert_start, moe.local_expert_end):
                self.assert_symmetric_int8(
                    f"model.layers.{i}.mlp.experts.{expert_idx}.",
