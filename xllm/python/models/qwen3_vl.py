...
class Qwen3VLForConditionalGeneration(PyModelBase):
    ...
    def load_weights(self, state_dicts: list, tp_rank: int, tp_size: int) -> None:
        ...
        self._load_vision_weights(loader)
        self._load_llm_weights(loader)
    ...
    def _load_vision_weights(self, loader: WeightLoader) -> None:
        ...
        def load(name: str) -> torch.Tensor:
            full = prefix + name
            state_dict = loader.find(full)
            if state_dict is None:
                raise KeyError(f"checkpoint tensor not found: {full}")
            return state_dict.get_tensor(full)

        def copy(param_name: str, ckpt_name: str | None = None) -> None:
            loader.copy_in("vision_model." + param_name, load(ckpt_name or param_name))
    ...
    def _load_llm_weights(self, loader: WeightLoader) -> None:
        ...
        cfg = self.text_cfg
        tp_rank = loader.tp_rank
        tp_size = loader.tp_size
        ...
        loader.copy_in("model.norm.weight", load("norm.weight"))

    def _load_lm_head(self, loader: WeightLoader) -> None:
