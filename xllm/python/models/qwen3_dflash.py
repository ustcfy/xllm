...
class DFlashModel(Qwen3Model):
    ...
    def write_context_kv(
        ...
        Attention-free, runs eager (outside the decode graph). Returns False on
        layer_synchronizer failure so the caller can fail-fast the PD-PUSH
        scatter.
        """
        projected = self.hidden_norm(self.fc(target_hidden))
        positions = positions.to(torch.int64).contiguous()
        cos_sin_cache = self.rotary.cos_sin_cache
        # Attention-free K/V-only context write: the draft never consumes Q, so
        # project just [K|V] and skip the discarded Q norm+RoPE lane. The [K|V]
        # scratch is reused across layers (same shape by construction).
        attn0 = self.layers[0].self_attn
        num_kv_heads = attn0.num_kv_heads
        head_dim = attn0.head_dim
        eps = attn0.k_norm.eps
        kv = projected.new_empty(projected.shape[0], 2 * num_kv_heads * head_dim)
        ...
        for i in range(len(self.layers)):
            k_weight = self._norm_weights[i]
            torch.matmul(projected, self._kv_weights_t[i], out=kv)
            k, v = kernels.fused_k_norm_rope(
                kv,
    ...
    def load_backbone_layers(
        ...
        *,
        ...
        # Per-layer k_norm.weight captured once; the write loop does not
        # retraverse nn.Module attribute chains per step.
        self._norm_weights: list[torch.Tensor] = []
        for i in range(self.cfg.n_layers):
            ...
            self._norm_weights.append(attn.k_norm.weight)
