...
class Qwen3_5ForCausalLM(PyModelBase):
    ...
    def load_weights(self, state_dicts: list, tp_rank: int, tp_size: int) -> None:
        ...
        def split_shard_cat(t: torch.Tensor) -> torch.Tensor:
            return torch.cat([loader.shard(p, 0) for p in t.split(qkv_sizes, dim=0)])
        ...
        for layer_id, layer_type in enumerate(cfg.layer_types):
            ...
            if layer_type == "full_attention":
                ...
                copy_in(
                    ...
                    split_shard_cat(tensor(linear + "in_proj_qkv.weight")),
                )
                ...
                copy_in(
                    ...
                    split_shard_cat(tensor(linear + "conv1d.weight").squeeze(1)),
                )
            ...
            if cfg.is_moe_layer(layer_id):
                ...
                gate_up = loader.shard(tensor(moe + "experts.gate_up_proj"), 0, *ep)
                gate, up = gate_up.chunk(2, dim=1)
                gate = loader.shard(gate, 1, *mtp)
                up = loader.shard(up, 1, *mtp)
                ...
                down = loader.shard(tensor(moe + "experts.down_proj"), 0, *ep)
                copy_in(target + "mlp.experts.w2", loader.shard(down, 2, *mtp))

                shared = moe + "shared_expert."
                copy_in(
                    target + "mlp.shared_expert.gate_up_proj.weight",
                    loader.pack_gate_up(shared),
                )
                copy_in(
                    target + "mlp.shared_expert.down_proj.weight",
                    loader.load_shard(shared + "down_proj.weight", 1),
                )
            else:
                copy_in(target + "mlp.gate_up_proj.weight", loader.pack_gate_up(source + "mlp."))
                copy_in(
                    target + "mlp.down_proj.weight",
                    loader.load_shard(source + "mlp.down_proj.weight", 1),
                )

        copy_in("model.norm.weight", tensor("norm.weight"))
        lm_head_name = "lm_head.weight"
        if cfg.tie_word_embeddings or not loader.has(lm_head_name):
            lm_head_name = "embed_tokens.weight"
        copy_in("lm_head.weight", loader.load_shard(lm_head_name, 0))
